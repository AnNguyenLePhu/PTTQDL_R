...
circle_circumference(2) # 12.56637
...
check_even_odd <- function(number) {
  ...
  if(number %% 2 == 0){
    return("Even") # Hoặc "Chẵn" nếu muốn tiếng Việt
  } else {
    return("Odd") # Hoặc "Lẻ" nếu muốn tiếng Việt
  }
}
check_even_odd(2) # "Even"
...
average_score <- function(scores) {
  if (!is.numeric(scores)) stop("Phải là số")
  scores <- scores[!is.na(scores)]
  if (length(scores) == 0 || any(scores < 0 | scores > 10)) stop("Dữ liệu không hợp lệ")
  mean(scores)
}

average_score(c(8, 7.5, 9, NA, 6)) # 7.625
average_score(c(11, 8)) # Dữ liệu không hợp lệ
...
# Tái sử dụng hàm combination từ bài trước
...
classify_student <- function(score) {
  # Validation
  if (!is.numeric(score) || score < 0 || score > 10) {
    stop("Điểm phải là số từ 0 đến 10")
  }
  
  # Phân loại và tính GPA 4.0
  if (score >= 9.0) {
    list(Grade = "Xuất sắc", GPA = 4.0)
  } else if (score >= 8.0) {
    list(Grade = "Giỏi", GPA = 3.5)
  } else if (score >= 6.5) {
    list(Grade = "Khá", GPA = 3.0)
  } else if (score >= 5.0) {
    list(Grade = "Trung bình", GPA = 2.0)
  } else {
    list(Grade = "Yếu", GPA = 1.0)
  }
}
...
calculate_net_salary <- function(base, allowance, days_worked, overtime_hours, tax_rate = 0.1) {
  # Giả định 1 tháng làm việc tiêu chuẩn là 22 ngày
  daily_rate <- base / 22
  hourly_rate <- (base / 22) / 8
  
  # Tính tổng thu nhập trước thuế
  gross_salary <- (daily_rate * days_worked) + allowance + (overtime_hours * hourly_rate * 1.5)
  
  # Tính lương ròng sau khi trừ thuế
  net_salary <- gross_salary * (1 - tax_rate)
  
  return(round(net_salary, 0))
}

# Ví dụ: Lương 15tr, phụ cấp 2tr, làm 20 ngày, tăng ca 5h, thuế 10%
calculate_net_salary(15000000, 2000000, 20, 5)
...
normalize_scores <- function(x) {
  x <- x[!is.na(x)]
  if(length(x) == 0) stop("Dữ liệu rỗng")
  (x - min(x)) / (max(x) - min(x)) * 100
}

normalize_scores(c(5,6,7,8,9))
# 0 25 50 75 100
...
analyze_students <- function(df) {
  ...
  if (!is.data.frame(df)) stop("Input phải là một Data Frame")
  
  summary_info <- list(
    total_students = nrow(df),
    average_age = mean(df$tuoi, na.rm = TRUE),
    average_score = mean(df$diem, na.rm = TRUE),
    highest_score = max(df$diem, na.rm = TRUE),
    lowest_score = min(df$diem, na.rm = TRUE)
  )
